...
> 用 AI 开发的人越来越多，但大多数人和 AI 的协作方式是「用完即走」：任务做完了，结论没验证，经验没沉淀，下一个 session 又从零开始。
> 这篇文章分享我怎么用三个 Agent Skill 把会话管成一条可复利的流水线——全部开源（Apache-2.0），附多方案安装教程。
...
**痛点 1：假收敛。**
让 Agent 批量修 bug 或写一份长文档，它做完会说「0 问题，完成」。结果你让它再仔细查一遍，立刻翻出五六个问题。不是它骗你——是它只做了**一次**浅层检查就下了结论。更糟的是，它还会给「看起来完整」的东西打包票。
...
这三个痛点的共同根源：**Agent 的 session 没有生命周期管理**。我把这当成工程问题来解——于是有了三个 Skill 组成的闭环。

## 二、解决方案：三 Skill 闭环

```mermaid
flowchart LR
    A["审查 Review<br/>5 轮深度复检 R0→R3<br/>3 独立 verifier + 对抗审查<br/>4 层过拟合防护"] -->|"收敛曲线 + ≥3 残余风险"| B["收尾 Wrap-up<br/>7 步流水线 + 6 面状态矩阵<br/>验证铁律: 声明 ≠ 事实<br/>sediment 经验沉淀"]
    B -->|"收尾报告 + 沉淀记录"| C["沉淀 Evolution<br/>3 问自检 / 11 维度复盘<br/>知识升级: experience→policy<br/>行动项 P0-P3 分流"]
    C -->|"复利经验 + 升级后的规则"| A
```

> 图源：[docs/images/session-loop-pipeline.svg](https://github.com/1273984347/agent-session-loop/blob/main/docs/images/session-loop-pipeline.svg)，可作掘金封面/插图。

三个 Skill 各自独立、可单独安装，也可组合成一条流水线：
...
想一步到位，装整合版 [agent-session-loop](https://github.com/1273984347/agent-session-loop)，三步编成一条完整流水线。

## 三、deep-review-loop：把「审查」变成真循环

核心是一条铁律：**绝不允许假收敛**。

普通审查是「审查 → 修复 → 验证」一次完事。这个 Skill 改成：
...
- **R2**：独立审计，明确禁止「自己审自己」（Self-audit ≠ 独立审计）。
- **R3**：必须写 ≥3 条残余风险 + 收敛曲线，即使 0 finding 也要写。

**反过拟合**是另一半。它知道审查可能「越修越多」，所以有 4 层防护：

1. **P2 残留 N**：P0/P1 必须为 0，但 P2（体验级）允许残留（比赛级 0 条 / 生产 3 条 / 原型 10 条）；
2. **边际收益 gate**：修复成本 > 问题危害 × 3，就标记「接受残留」；
3. **过拟合警报**：连续几轮问题数不降反升，或回归率 >30%，立刻 STOP 报告而不是硬修；
4. **严重度门槛**：P3 及以下不报，避免噪声。

还有个细节值得抄：**每个 finding 必须附工具调用证据**。subagent 说「这个目录不存在」必须附 LS 输出，说「某行有问题」必须附 Read 输出，连「0 finding」也要附证据证明你真查了。这直接堵死了 subagent 编结论的路。
...
```text
1. memory 健康检查      2. memory 审计（5 phase）+ 6 面状态矩阵
3. 文件数同步检查       4. 文档同步 spot-check + work-log
5. 经验沉淀（sediment） 6. 4 步 verify
7. memory 层复查 + 反向审查
```
...
为什么？「Edit 成功」不等于「文件真改了」——old_string 不匹配会静默失败，并行 Edit 会互相覆盖，改完不验证就是白改。我在实际项目里被这个坑坑了十几次，最后固化成铁律：**声明 ≠ 事实，验证才算数**。

## 五、self-evolution：把「复盘」变成复利

最后一步是把 session 的经验变成可复用规则。两种模式：
...
不想管理三个 skill？[agent-session-loop](https://github.com/1273984347/agent-session-loop) 把三步编成一条流水线。每阶段的产出就是下一阶段的输入：审查的 residual risk 是收尾的验证清单，收尾的 sediment 是复盘的分析素材，复盘的规则升级喂回下一个 session。**上一轮的结论变成下一轮的起点，这就是复利。**
...
四个仓库都已配置 `.claude-plugin/marketplace.json`，开箱即用。
...
**1. 遵循标准格式。** SKILL.md 的 frontmatter：`name` 必须等于目录名、小写+连字符、≤64 字符；`description` 双语、祈使句（「Use when… / 当…时使用」）方便 agent 触发；加 `license: Apache-2.0`。正文按**渐进披露**组织：SKILL.md 保持 <500 行，细节拆到 `references/`，agent 按需加载。
...
**3. 为 MCP 预留扩展位。** Skill 和 MCP 互补：MCP 提供外部工具/数据连接，Skill 教 Agent 如何编排这些工具的复杂工作流。所以这几个 skill 把 MCP 当**可选依赖**——有对应 MCP server 就用，没有就回退内建工具，绝不绑死。
...
如果你也在和 Agent 长期协作，试试把「审查、收尾、沉淀」做成自己的闭环——不用我的方案，但一定要有自己的闭环。

---

*标签建议：Agent Skills · AI Agent · Claude Code · 效率工具 · 开源*
