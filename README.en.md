...
[![version](https://img.shields.io/github/v/release/1273984347/agent-session-loop)](https://github.com/1273984347/agent-session-loop/releases/latest)
...
- **Tools**: file search (Grep/Read/LS) + shell (PowerShell examples; macOS/Linux use bash/zsh equivalents — see the cross-platform command table in SKILL.md); subagent/task spawning is **optional** — without it, the skill falls back to the degradation mode (see the "no-subagent degradation mode" section in SKILL.md: serial instead of parallel, explicitly marked `degraded (no-subagent)`).
